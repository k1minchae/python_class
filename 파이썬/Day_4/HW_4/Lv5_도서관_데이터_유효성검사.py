...
error_msg = []
def is_validation():    # 올바른 데이터로 이루어졌는지 확인
    global error_msg 
    for user_datas in user_data:
        if user_datas['blood_group'] not in blood_types:
            error_msg.append(user_datas)
            user_datas['blood_group'] = None
        if '@' not in user_datas['mail']:
            error_msg.append(user_datas)
        if (len(user_datas['name']) > 30 and len(user_datas['name']) < 2):
            error_msg.append(user_datas)
            user_datas['name'] = None
        if user_datas['website'] == False:
            error_msg.append(user_datas)
            user_datas['website'] = None
        if user_datas['company'] in black_list:
            error_msg.append(user_datas)
            user_datas['company'] = 'blocked'
    if error_msg:
        return error_msg
    else: 
        return None
...
def create_user():
    ...
    for errors in error_msg:
        if errors['company'] != 'blocked':
            user_list.append(errors)
    return user_list
        
        
is_validation()
create_user()
print(f'잘못된 데이터로 구성된 유저의 수는 {len(error_msg)} 입니다.')
